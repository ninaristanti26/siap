...
if (isset($_GET['id_ao'])) {
    ...
    // Ambil kode_cabang dari tabel AO
...
<div class="main-panel">
  <div class="content-wrapper">
    ...
    <div class="mb-3">
      <button type="button" class="btn btn-primary btn-sm text-white" onclick="toggleFormDebitur()">+ Tambah Data</button>
    </div>

    <?php include "pilih_data.php"; ?>
    ...
    <div class="table-responsive">
      <table class="table table-hover table-bordered" id="dataTables-example" width="108%">
        ...
        <?php
        $query = "SELECT norek,
                    MAX(debitur_npl.nama_debitur) AS nama_debitur,
                    MAX(debitur_npl.alamat_npl) AS alamat_npl,
                    MAX(debitur_npl.plafon_npl) AS plafon_npl,
                    MAX(debitur_npl.id_deb) AS id_deb,
                    MAX(ao.nama_ao) AS nama_ao,
                    MAX(cabang.nama_cabang) AS nama_cabang,
                    MAX(debitur_npl.tgl_data) AS tgl_data
                  FROM debitur_npl
                  JOIN ao ON debitur_npl.id_ao = ao.id_ao
                  JOIN cabang ON ao.kode_cabang = cabang.kode_cabang
                  WHERE ao.id_ao = ?
                  AND DATE_FORMAT(debitur_npl.tgl_data, '%Y-%m') = ?
                  GROUP BY norek
                  LIMIT 0, 25;
        ";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param("ss", $id_ao, $selectedBulan);
        $stmt->execute();
        $result = $stmt->get_result();
        $no = 1;

        while ($data = $result->fetch_assoc()):
            $norek        = isset($data['norek']) ? htmlspecialchars($data['norek']) : '';
            $nama_debitur = isset($data['nama_debitur']) ? htmlspecialchars($data['nama_debitur']) : '';
            $alamat_npl   = isset($data['alamat_npl']) ? htmlspecialchars($data['alamat_npl']) : '';
            $plafon_npl   = number_format($data['plafon_npl'], 0, ',', '.');
            $tgl_data     = isset($data['tgl_data']) ? htmlspecialchars($data['tgl_data']) : '';
        ?>
        <tr>
          ...
          <td class="text-center">
            <a href="history_pipeline.php?norek=<?= urlencode($norek) ?>&bulan=<?= urlencode(substr($tgl_data, 0, 7)) ?>" class="btn btn-sm btn-primary">
              Selengkapnya
            </a>
          </td>
        </tr>
