...
<div class="main-panel">
  <div class="content-wrapper">
    ...
    <hr>
    <?php include "pilih_data_pusat.php"; ?>
    ...
    <div class="table-responsive">
      <table class="table table-hover table-bordered" id="dataTables-example" width="100%">
        ...
        $query = "
          ...
          WHERE DATE_FORMAT(debitur_npl.tgl_data, '%Y-%m') = ?
          GROUP BY norek
        ";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param("s", $selectedBulan);
        $stmt->execute();
        $result = $stmt->get_result();
        $no = 1;

        while ($data = $result->fetch_assoc()):
            $id_ao = htmlspecialchars($data['id_ao']);
        ...
        <tr>
          ...
          <td class="text-center"><?= $id_ao ?></td>
          <td class="text-center"><?= $norek ?></td>
          <td><?= $nama_debitur ?></td>
          <td><?= $alamat_npl ?></td>
          <td class="text-right"><?= $plafon_npl ?></td>
          <td class="text-center">
            <a href="history_pipeline_pusat?norek=<?= urlencode($norek) ?>&bulan=<?= urlencode(substr($tgl_data, 0, 7)) ?>" class="btn btn-sm btn-primary">
              Selengkapnya
            </a>
          </td>
        </tr>
